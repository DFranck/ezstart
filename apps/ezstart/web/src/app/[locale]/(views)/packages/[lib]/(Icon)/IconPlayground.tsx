...
const IconPlayground = ({ title }: Props): React.JSX.Element => {
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return iconSuggestions
      ...
      .slice(0, 15) // limite a 15
  }, [query])

  const handleSelect = (suggestion: string) => {
    setPlaygroundValue(suggestion as KnownIconName)
    setQuery(suggestion)
    setIsFocused(false) // masque la liste apres selection
  }

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && filteredSuggestions.length > 0) {
      // si Enter -> prend la premiere suggestion
