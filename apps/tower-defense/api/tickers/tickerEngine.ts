...
import type { ActiveMob, Game, InGamePlayer } from '@tower-defense/types'
...
function moveMobs(activeMobs: ActiveMob[], players: InGamePlayer[]): ActiveMob[] {
  ...
  return activeMobs.map(mob => {
    ...
    console.log(`[moveMobs] Mob ${mob.id}: pathIndex=${mob.pathIndex}, pathLength=${path.length}, position=(${mob.position.x}, ${mob.position.y})`)
