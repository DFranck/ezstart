/* path: /app/[locale]/landing-sections.tsx */
'use client'
...
export function HeroSection({ t, currentYear }: HeroSectionProps) {
  return (
    <Section size={'full'} className="pt-16 relative overflow-hidden">
      <Div className="absolute inset-0 -z-10">
        <Div className="absolute inset-0 animate-pulse" style={getGradientWithOpacity(15, 'br')} />
        <Div
...
/* ── Features (Process Flow) Section ── */
interface FeaturesSectionProps {
  t: (key: string) => string
  features: Array<{ icon: string; title: string; description: string }>
}

export function FeaturesSection({ t, features }: FeaturesSectionProps) {
  return (
...
/* ── Benefits + Five Elements Section ── */
interface BenefitsSectionProps {
  t: (key: string, params?: Record<string, string | number>) => string
  benefits: Array<{ key: string; icon: string; color: string }>
  fiveElements: Array<{ name: string; icon: string; color: string }>
}

export function BenefitsSection({ t, benefits, fiveElements }: BenefitsSectionProps) {
  return (
...
interface ComparisonSectionProps {
  t: (key: string, params?: Record<string, string | number>) => string
  currentYear: number
  comparisonRows: Array<{ key: string; free: boolean; premium: boolean; isNew?: boolean }>
}

export function ComparisonSection({ t, currentYear, comparisonRows }: ComparisonSectionProps) {
  return (
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            ...
            <tr className="bg-muted/50 font-bold">
              ...
              <td className="p-4 text-center">
                <Span className="text-sm font-bold text-success">{t('comparison.total.ours')}</Span>
              </td>
            ...
            <Div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10 max-w-3xl mx-auto">
              <Div className="flex flex-col items-center">
                <Icon
                  ...
                  style={{ color: '#D4A017' }}
                />
                <Span className="text-sm font-semibold text-foreground">{t('cta.advantage1')}</Span>
              </Div>
              <Div className="flex flex-col items-center">
                <Icon name="lucide:Bot" className="w-8 h-8 mb-2" style={{ color: '#C41E3A' }} />
                <Span className="text-sm font-semibold text-foreground">{t('cta.advantage2')}</Span>
              </Div>
              <Div className="flex flex-col items-center">
                <Icon name="lucide:Zap" className="w-8 h-8 mb-2" style={{ color: '#2E7D32' }} />
                <Span className="text-sm font-semibold text-foreground">{t('cta.advantage3')}</Span>
              </Div>
            </Div>
