                      ...
                      return (
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <P className="text-xs">
                                Grind: +{grindRange.min}–{grindRange.max}{' '}
                                {formatStatLabel(stat.type)}
                                {sfx}
                              </P>
                            </TooltipContent>
          ...
          (() => {
            ...
            if (rune.innateStat && innateTier) {
              if (innateTier === 'S' || innateTier === 'A')
                ...
                positives.push(
                  `Innate ${formatStatLabel(rune.innateStat.type)} ${innateTier}-tier correcte`
                )
            }
            ...
            if (setStrengthTier === 'S' || setStrengthTier === 'A')
              ...
              negatives.push(
                `Set ${rune.set} (${setStrengthTier}-tier, +${setBonus}% sur les seuils)`
              )
              ...
              (() => {
                ...
                return (
                  <Div className="bg-muted/10 border border-border rounded p-2 space-y-2 font-mono text-[9px] leading-tight">
                    ...
                    {breakdown && maxDivisor && (
                      <Div className="space-y-0.5">
                        <P className="font-bold text-[10px] text-foreground">
                          Set-Weighted Efficiency Breakdown
                        </P>
                        {breakdown.map((item, idx) => {
                          ...
                          const grindColor =
                            item.grindBonus > 1 ? 'text-success-foreground' : 'text-destructive'
                          const contribColor =
                          ...
                          return (
                            <Div key={idx} className="space-y-0.5 pl-1">
                              ...
                              <P className="text-muted-foreground/60 pl-2">
                                ratio = ({item.value}/{item.maxPossible}) x {item.rolls} ={' '}
                                {item.ratio}
                              </P>
                              <P className="text-muted-foreground/60 pl-2">
                        ...
                        <Div className="border-t border-dashed border-border/30 mt-1 pt-1 pl-1 space-y-0.5">
                          <P className="text-muted-foreground">
                            Total weighted sum:{' '}
                            {Math.round(breakdown.reduce((s, b) => s + b.contribution, 0) * 100) /
                              100}
                          </P>
                          <P className="text-muted-foreground">
                          ...
                          <P className="text-foreground font-medium">
                            ...
                            {Math.round(breakdown.reduce((s, b) => s + b.contribution, 0) * 100) /
                              100}{' '}
                            / {maxDivisor} x 100 = {analysis.setWeightedEfficiency}%
                          </P>
                        </Div>
                      </Div>
                    )}
                    ...
                    <Div className="space-y-0.5">
                      <P className="font-bold text-[10px] text-foreground">
                        Progressive Advice (all profiles)
                      </P>
                      {(() => {
                        const profiles: PlayerProfile[] = ['early', 'mid', 'late']
                        const levelKey = Math.min(Math.floor(rune.level / 3) * 3, 12) as
                          | 0
                          | 3
                          | 6
                          | 9
                        ...
                        const potentialWithPenalties =
                          adjPotentialApi ??
                          (analysis.potentialEfficiency ?? currentEff) + penaltiesTotal
                                ...
                                <tr>
                                  ...
                                  {profiles.map(p => {
                                    ...
                                    const prob = Math.min(
                                      ...
                                      Math.max(
                                        5,
                                        Math.round((1 - adjPotential / finalThresh) * 100)
                                      )
                                    )
                                    return (
                                      <td key={p} className="text-center px-1">
                      ...
                      <Div className="space-y-0.5">
                        ...
                        {bestOptim.gemTarget ? (
                          <>
                            ...
                            <P className="text-muted-foreground/60 pl-2">
                              Reason: {bestOptim.gemTarget.reason}
                            </P>
                            {rune.subStats.map((sub, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {isGrindable ? `grindable(${grindBonus})` : `non-grind(0)`} +{' '}
                                  {powerupRolls} rolls({rollBonus}) = {total}
                                  {isTarget ? ' <-- LOWEST' : ''}
                                </P>
                              )
                            })}
                            ...
                            <P className="text-muted-foreground/60 pl-2">
                              Rolls lost: {bestOptim.rollsLost} | Post-optim score:{' '}
                              {bestOptim.postOptimScore}
                            </P>
                          </>
                        ) : (
