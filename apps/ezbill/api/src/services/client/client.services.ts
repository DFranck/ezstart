...
export async function createClientService(data: BillingClient): Promise<Client> {
...
export async function getClientByIdService(id: string, userId?: string): Promise<Client | null> {
  const ClientModel = await getClientModel()
...
export async function hardDeleteClientService(id: string, userId?: string): Promise<Client | null> {
  const ClientModel = await getClientModel()
...
export async function restoreClientService(id: string, userId?: string): Promise<Client | null> {
  const ClientModel = await getClientModel()
  const filter: Record<string, unknown> = { _id: id }
  if (userId) {
    filter.userId = userId
...
export async function softDeleteClientService(id: string, userId?: string): Promise<Client | null> {
  const ClientModel = await getClientModel()
  const filter: Record<string, unknown> = { _id: id }
  if (userId) {
    filter.userId = userId
