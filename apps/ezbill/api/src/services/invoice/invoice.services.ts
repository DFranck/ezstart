import { CreateInvoice, GetInvoicesQuery, Invoice, UpdateInvoice } from '@ezbill/types'
...
export async function createInvoiceService(data: CreateInvoice): Promise<Invoice> {
...
export async function getInvoicesService(query: GetInvoicesQuery): Promise<Invoice[]> {
  const InvoiceModel = await getInvoiceModel()
...
export async function getInvoicesPaginatedService(
  query: GetInvoicesQuery
): Promise<PaginatedResult<Invoice>> {
  const InvoiceModel = await getInvoiceModel()
  return findWithQueryPaginated(InvoiceModel, query, {
    ...(query.status ? { status: query.status } : {}),
    ...(query.clientId ? { clientId: query.clientId } : {}),
  })
}

export async function getInvoiceByIdService(id: string): Promise<Invoice | null> {
  const InvoiceModel = await getInvoiceModel()
  const doc = await InvoiceModel.findById(id)
  return doc ? toApiObject<Invoice>(doc) : null
}

export async function softDeleteInvoiceService(id: string): Promise<Invoice | null> {
  const InvoiceModel = await getInvoiceModel()
...
export async function hardDeleteInvoiceService(id: string): Promise<Invoice | null> {
  const InvoiceModel = await getInvoiceModel()
...
export async function restoreInvoiceService(id: string): Promise<Invoice | null> {
  const InvoiceModel = await getInvoiceModel()
