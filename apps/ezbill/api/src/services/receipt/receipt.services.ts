import { CreateReceipt, GetReceiptsQuery, Receipt, UpdateReceipt } from '@ezbill/types'
...
export async function createReceiptService(data: CreateReceipt): Promise<Receipt> {
...
export async function getReceiptsService(query: GetReceiptsQuery): Promise<Receipt[]> {
  const ReceiptModel = await getReceiptModel()
...
export async function getReceiptsPaginatedService(
  query: GetReceiptsQuery
): Promise<PaginatedResult<Receipt>> {
  const ReceiptModel = await getReceiptModel()
  return findWithQueryPaginated(ReceiptModel, query, {
    ...(query.status ? { status: query.status } : {}),
    ...(query.clientId ? { clientId: query.clientId } : {}),
  })
}

export async function getReceiptByIdService(id: string): Promise<Receipt | null> {
  const ReceiptModel = await getReceiptModel()
  const doc = await ReceiptModel.findById(id)
  return doc ? toApiObject<Receipt>(doc) : null
}

export async function softDeleteReceiptService(id: string): Promise<Receipt | null> {
  const ReceiptModel = await getReceiptModel()
...
export async function hardDeleteReceiptService(id: string): Promise<Receipt | null> {
  const ReceiptModel = await getReceiptModel()
...
export async function updateReceiptService(
  ...
  const ReceiptModel = await getReceiptModel()
  const totals = calculateTotals(data.items ?? [], data.taxRate ?? 0)
  const doc = await ReceiptModel.findByIdAndUpdate(id, { ...data, ...totals }, { new: true })
  return doc ? toApiObject<Receipt>(doc) : null
}

export async function restoreReceiptService(id: string): Promise<Receipt | null> {
  const ReceiptModel = await getReceiptModel()
