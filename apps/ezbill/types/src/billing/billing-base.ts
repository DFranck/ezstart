...
export const billingTypeEnum = z
  .enum(['itemized', 'flat-rate'])
  .describe('Type of billing: itemized (line items) or flat-rate (single description + price)')
...
export const baseBillingDocSchemaRaw = z.object({
  ...
  items: z
    .array(baseLineItemSchema)
    .optional()
    .describe('List of billing line items (required for itemized type)'),
  description: z
    .string()
    .optional()
    .describe('Flat-rate description (required for flat-rate type)'),
  flatRateAmount: z
    .number()
    .min(0)
    .optional()
    .describe('Flat-rate amount (required for flat-rate type)'),
  ...
  paymentMethodIds: z
    .array(z.string())
    .optional()
    .describe('Payment method identifiers to display on invoice'),
  aiConversationHistory: z
    ...
    .optional()
    .describe('AI assistant conversation history for this invoice/quote'),
})
...
export function withBillingOutputFields<T extends ZodRawShape>(schema: ZodObject<T>) {
...
export function getBillingDocsQuerySchema<T extends ZodEnum<[string, ...string[]]>>(statusEnum: T) {
  return listingQuerySchema.extend({
    userId: z.string().optional().describe('Filter by user ID (auto-filled from auth)'),
    ...
    deletedOnly: z
      .enum(['true', 'false'])
      .optional()
      .describe('Only return soft-deleted documents'),
    includeDeleted: z.enum(['true', 'false']).optional().describe('Include soft-deleted documents'),
  })
}
