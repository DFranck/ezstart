...
import DashboardTabs, { TabConfig } from '@/components/DashboardTabs'
...
import { groupByField, groupByFirstLetter } from '@/utils/grouping'
...
const DashboardPageNew = () => {
  ...
  const clientsByLetter = useMemo(
    () => groupByFirstLetter(sortedClients, client => client.clientName),
    [sortedClients]
  )

  // Group companies by first letter
  const companiesByLetter = useMemo(
    () => groupByFirstLetter(companies || [], company => company.companyName),
    [companies]
  )
  ...
  const paymentMethodsByType = useMemo(
    () =>
      groupByField(paymentMethods || [], method => method.type, {
        bank_transfer: { label: 'Bank Transfers', icon: 'lucide:Building' },
        crypto_wallet: { label: 'Crypto Wallets', icon: 'lucide:Wallet' },
        paypal: { label: 'PayPal', icon: 'lucide:CreditCard' },
        wise: { label: 'Wise', icon: 'lucide:CreditCard' },
        revolut: { label: 'Revolut', icon: 'lucide:CreditCard' },
        stripe: { label: 'Stripe', icon: 'lucide:CreditCard' },
        other: { label: 'Other', icon: 'lucide:HelpCircle' },
      }),
    [paymentMethods]
  )
        ...
        actions: [
          {
            ...
            gradient: 'bg-gradient-client hover:bg-gradient-client-hover',
            onClick: () => setIsClientModalOpen(true),
          },
        ],
        content: (
          <GroupedSection
            groups={clientsByLetter}
            renderItem={client => (
              <ClientCard
                client={client}
            ...
            useAccordion={clients && clients.length > 10}
            emptyState={{
              icon: 'lucide:Users',
        ...
        actions: [
          {
            label: 'Add Company',
            icon: 'lucide:Plus',
            gradient: 'bg-gradient-company hover:bg-gradient-company-hover',
            onClick: () => setIsCompanyModalOpen(true),
          },
        ],
        content: (
          <GroupedSection
            groups={companiesByLetter}
            renderItem={company => (
              <CompanyCard
                company={company}
            ...
            useAccordion={companies && companies.length > 10}
            emptyState={{
              icon: 'lucide:Building2',
              iconBg: 'bg-gradient-company-light text-ezbill-company',
              title: 'No companies yet',
              description: 'Add your business profile and billing information',
            }}
          />
        ),
      },
      {
        ...
        actions: [
          {
            ...
            icon: 'lucide:Plus',
            gradient: 'bg-gradient-payment hover:bg-gradient-payment-hover',
        ...
        content: (
          <GroupedSection
            ...
            emptyState={{
              icon: 'lucide:CreditCard',
              iconBg: 'bg-gradient-payment-light text-ezbill-payment',
  ...
  return (
    <Main>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        ...
        {/* Dashboard Tabs - NOUVELLE APPROCHE */}
        <DashboardTabs tabs={dashboardTabs} defaultTab="clients" />
      </div>
