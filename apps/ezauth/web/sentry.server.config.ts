 ...
 *
 * NOTE: this is the Next.js server runtime (RSC + route handlers). The
 * standalone Express API (`apps/ezauth/api`) initializes Sentry separately
 * via `@sentry/node-core` + `initSentry()` from `@ezstart/api-core`.
 *
