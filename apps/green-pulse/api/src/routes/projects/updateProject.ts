...
import { ProjectSchema, UpdateProjectRequestSchema, ApiResponseSchema } from '@green-pulse/types'
...
docRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      const project = await Project.findByIdAndUpdate(req.params.id, validation.data, { new: true })
