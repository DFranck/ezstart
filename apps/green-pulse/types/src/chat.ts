...
export const ChatRequestSchema = z
  .object({
    ...
    providerId: z
      ...
      .optional()
      .describe(
        'AI provider ID (e.g., "gemini-flash", "openai-gpt4"). Defaults to "gemini-flash" if not specified'
      ),
    session_id: z
      .string()
      .optional()
      .describe('Session ID for conversation continuity (deprecated, use userId)'),
    ...
    extract_esg: z
      .boolean()
      .default(false)
      .describe('Whether to extract ESG data from the message'),
    userId: z.string().optional().describe('User ID from EZAuth (if authenticated)'),
    locale: z
      .string()
      .optional()
      .describe('User locale (e.g., "en", "fr", "vi") — AI responds in this language'),
  })
...
export const TextExtractionRequestSchema = z
  .object({
