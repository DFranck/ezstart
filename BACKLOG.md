...
- [ ] **PAY_DOCS_DEMO_SANDBOX-001** (#178) 🟠 P1 — PARTIAL (only `apps/ezauth/api/src/scripts/seed-pay-docs-demo-app.ts` written — creates the `_pay-docs-demo` Application doc in ezauth DB, idempotent, ready to wire). Remaining work: package.json script entry, ezpay-side counterpart seed for plans/payments/donations, `middleware/check-pay-demo-quotas.ts`, `services/pay-docs-demo-reset.service.ts` 24h cron, isolation tests. Re-launch in next session.
