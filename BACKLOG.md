...
- [x] **Migrate 6 APIs from `@ezstart/express-core` → `@ezstart/api-core`** — DONE 2026-04-16. 279 source files migrated, ESLint rule `no-express-core` blocks regressions, express-core removed from all API deps.
- [x] **Fix OpenAPI params schemas crash** — DONE 2026-04-16. Added `extendZodWithOpenApi(z)` to api-core + fixed 23 route files `.describe()` → `.openapi()`.
- [x] **SDK 3-layer architecture (standard.md 0bis)** — DONE 2026-04-16. api-sdk (core/react split), pay-sdk (core/react/components), auth-sdk (core/react/components). All packages standard-compliant.
- [ ] **npm publish setup** — Setup `npm publish --access public` for all `@ezstart/*` packages. Changesets for versioning + changelog. GitHub Action on tag `v*.*.*` auto-publishes modified packages. Packages: api-contracts, api-core, api-sdk, auth-sdk, pay-sdk, ui (phase 1).
