...
- [ ] **Migrate 6 APIs from `@ezstart/express-core` → `@ezstart/api-core`** — `@ezstart/api-core` est construit + publishable mais aucune des 6 APIs (ezauth, ezbill, ezpay, ezstart, green-pulse, gacha-analyzer) ne l'utilise. Équivalent côté serveur du travail fait pour `api-sdk` côté client. Migration : `createApp({ apiApp })` → `createApiServer({ app })`, `apiRouterWithDoc` → `createDocRouter`, middleware factories (auth/cors/rate-limit/validate), `DbConnector` interface. Tester chaque API en dev puis staging avant merge. Permettra de supprimer `@ezstart/express-core` (gros chantier ~3-5j).
- [ ] **Reduce OpenAPI debug log flood** — `packages/express-core/src/openapi/check-missing-descriptions.ts:22` émet un `logger.debug()` par Zod schema sans description. Flood au boot de chaque API (~35 lignes pour ezauth). Options : (A) downgrade à `trace` level, (B) aggregate en 1 warning agrégé au lieu de N logs, (C) ajouter les descriptions manquantes aux schemas Zod (propre mais long). Fait partie du chantier plus large de migration vers `api-core` qui aura son propre système OpenAPI revu.
