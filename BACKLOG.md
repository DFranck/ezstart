...
- [ ] **PHASE_2_AMBIGUOUS_REVIEW-001** 🟠 P1 — User decision needed on 3 candidates flagged AMBIGUOUS in audit `tmp/audit-full-registry-matrix.md`: `MaintenanceBanner` (split hook/UI vs keep auth-sdk?), `UsageBadge` (decouple useApiKeyUsage hook?), `DevModeBanner` (KEEP-EXPORT or SAFE-INTERNAL?).
- [ ] **PHASE_1_REMOVAL-001** 🟡 P2 — 2026-08-01: remove deprecated re-exports from `@ezstart/auth-sdk` (`AuthErrorBanner`, `ScopeContextIndicator`, `PasswordStrength`, `TurnstileWidget`). Bump major version (breaking change). Verify all consumer apps already import from `@ezstart/ui/components` / `@ezstart/api-sdk/integrations` before removal.
...
- [ ] **DOCS-COMPONENTS-ADMIN-INTERNAL-TOGGLE-001** 🟡 P2 — Add an admin-only "Show internal components" switch on `/docs/components` landing (visible only when `user.globalRoles.includes('superadmin')`). When enabled, render the `@internal` / shell components hidden by default. Requires `getServerAuth` SSR companion already wired in the app. Skipped during REFACTOR_DOCS_COMPONENTS-001 to keep the PR scoped.
