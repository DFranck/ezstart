 ...
 */
export function passwordResetTemplate(data: PasswordResetData, ctx: EmailContext): RenderedEmail {
  const dict = getLocaleDict(ctx.locale)
  const section = dict.passwordReset
  ...
  const html =
    ...
    renderLayout({
      ...
      bodyInnerHtml: `
        ...
        <p><a class="email-cta" href="${escapeHtml(data.resetUrl)}">${escapeHtml(ctaLabel)}</a></p>
