...
function SignInCardInner({
  ...
  // submit button (rendered in the Card footer) can show its own spinner
  ...
  // External submit button rendered in the Card footer (sticky at the bottom
  // so it stays visible even when the form body scrolls). Wired to the form
  // via HTML `<button form="...">` association — `<SignInForm>`'s `onSubmit`
  // still runs the actual submission. `isSubmitting` mirrors the form's
  // internal `loading` flag (lifted via `onSubmittingChange`).
