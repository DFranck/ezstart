/**
 * Safe server-only guard for @ezstart/auth-sdk/server.
 *
 ...
 *     undefined or stripped, so the guard throws to prevent cookie / token
 *     leaks. Also covers React Native and other JS hosts that lack a Node
 *     process object.
 *
 ...
 *
 * Files in `packages/auth-sdk/src/server/` import this module via a
...
if (!isNode) {
  throw new Error(
    '@ezstart/auth-sdk/server cannot be imported from a Client Component or browser bundle. ' +
      'Use @ezstart/auth-sdk or @ezstart/auth-sdk/components instead.'
  )
}
export {}
