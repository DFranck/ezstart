...
/**
 * User-overridable strings for `<PurchaseCancelTemplate>`.
 * All fields are optional — English defaults are used when omitted.
 */
export interface PurchaseCancelTemplateTexts {
...
const DEFAULT_TEXTS: Required<PurchaseCancelTemplateTexts> = {
 ...
 *
 * Originally `PurchaseCancelPage` from `@ezstart/pay-sdk`.
 *
 * @example
 * ```tsx
 * 'use client'
 * import { PurchaseCancelTemplate } from '@ezstart/ui/components'
 *
 * export default function Page() {
 *   return <PurchaseCancelTemplate tryAgainHref="/en/purchase" />
 * }
 * ```
 */
export function PurchaseCancelTemplate({
  tryAgainHref = '/',
  backHomeHref = '/',
  texts,
}: PurchaseCancelTemplateProps): React.ReactElement {
  ...
  return (
    <CheckoutCallbackBase
      ...
      actions={[
        { href: tryAgainHref, label: t.primaryCtaLabel, icon: 'lucide:ShoppingBag' },
        {
          href: backHomeHref,
...
PurchaseCancelTemplate.displayName = 'PurchaseCancelTemplate'
