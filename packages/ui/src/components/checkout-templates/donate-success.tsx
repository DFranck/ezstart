...
/**
 * User-overridable strings for `<DonateSuccessTemplate>`.
 * All fields are optional — English defaults are used when omitted.
 */
export interface DonateSuccessTemplateTexts {
...
const DEFAULT_TEXTS: Required<DonateSuccessTemplateTexts> = {
  title: 'Thank You!',
...
export interface DonateSuccessTemplateProps {
  ...
  texts?: DonateSuccessTemplateTexts
}
 ...
 *
 * Originally `DonateSuccessPage` from `@ezstart/pay-sdk`.
 *
 * @example
 * ```tsx
 * 'use client'
 * import { DonateSuccessTemplate } from '@ezstart/ui/components'
 *
 ...
 * ```
 */
export function DonateSuccessTemplate({
  redirectTo,
  redirectDelayMs = 0,
  onComplete,
  texts,
}: DonateSuccessTemplateProps): React.ReactElement {
...
DonateSuccessTemplate.displayName = 'DonateSuccessTemplate'
