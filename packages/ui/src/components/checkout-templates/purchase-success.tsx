...
/**
 * User-overridable strings for `<PurchaseSuccessTemplate>`.
 * All fields are optional — English defaults are used when omitted.
 */
export interface PurchaseSuccessTemplateTexts {
...
const DEFAULT_TEXTS: Required<PurchaseSuccessTemplateTexts> = {
...
export interface PurchaseSuccessTemplateProps {
  ...
  texts?: PurchaseSuccessTemplateTexts
}
 ...
 *
 * Originally `PurchaseSuccessPage` from `@ezstart/pay-sdk`.
 *
 * @example
 * ```tsx
 * 'use client'
 * import { PurchaseSuccessTemplate } from '@ezstart/ui/components'
 *
 * export default function Page() {
 *   return <PurchaseSuccessTemplate />
 * }
 * ```
 */
export function PurchaseSuccessTemplate({
  redirectTo,
  redirectDelayMs = 0,
  onComplete,
  texts,
}: PurchaseSuccessTemplateProps): React.ReactElement {
...
PurchaseSuccessTemplate.displayName = 'PurchaseSuccessTemplate'
