...
const loadingPromises = new Map<string, Promise<React.ComponentType<LucideProps>>>()
  ...
  const loadPromise = (async () => {
    switch (prefix) {
      case 'lucide': {
        ...
        const IconComponent =
          (mod[iconName as keyof typeof mod] as React.ComponentType<LucideProps>) || mod.HelpCircle
      ...
      case 'fa': {
        ...
        const IconComponent =
          (mod[iconName as keyof typeof mod] as React.ComponentType<LucideProps>) ||
          (mod.FaCircle as React.ComponentType<LucideProps>)
        iconCache.set(cacheKey, IconComponent)
        return IconComponent
      }
      case 'custom': {
        const component = customIconMap[iconName as keyof typeof customIconMap]
        if (!component) {
      ...
      default: {
        logger.warn(`Unknown icon library: ${prefix}, falling back to Lucide 'HelpCircle'`)
        const mod = await import('lucide-react')
...
export function Icon({
  ...
  if (!name) {
    logger.warn(`[Icon] name is falsy → rendering fallback`, {
      name,
      type: typeof name,
      isNull: name === null,
  ...
  return (
    ...
    >
      <DynamicIcon
        ...
        {...ariaAttributes}
        className={[tailwindSize, className, spin && 'animate-spin'].filter(Boolean).join(' ')}
        style={finalStyle}
      >
