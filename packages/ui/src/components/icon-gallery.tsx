...
export const IconGallery = ({
  ...
  const filtered = icons.filter(({ name }) => name.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <Tag as="section">
      ...
      <div
        ref={parentRef}
        className={`relative overflow-y-auto border rounded ${fullHeight ? 'h-full' : ''}`}
        style={{
      ...
      >
        ...
        >
          {rowVirtualizer.getVirtualItems().map(virtualRow => {
            ...
            const itemsInRow = filtered.slice(startIndex, startIndex + columnCount)
