...
export const ThreadMessages = React.memo(function ThreadMessages<
  TMessage extends ThreadMessageType = ThreadMessageType,
>({
  messages,
  loading = false,
  streamingText = '',
  ...
  const shouldShowLoading =
    (loading || streamingText) && (messages.length > 0 || (isNewThread && streamingText))
  ...
  return (
    <div className="flex flex-col h-full justify-end w-full max-w-4xl mx-auto">
      {messages.map((msg, index) => {
        ...
        const key = msg.id ?? `${msg.timestamp ?? 't'}-${index}`
      ...
      {shouldShowLoading && (
        <ThreadMessage role="ai" aiBubbleClassName={aiBubbleClassName}>
          {streamingText ? (
            ...
            >
              {loadingText}
              <span className="animate-bounce text-lg font-bold" aria-hidden="true">
                .
              </span>
              <span
