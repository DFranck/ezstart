 ...
 *
 * Output: apps/ezstart/web/.../inspector/registry.generated.ts (gitignored)
 * Source of truth: packages/ui/src/components/**
 *
 * CommonJS (.cjs) because `@ezstart/ui` is an ESM package; this keeps
 * the existing `require(...)` style without an ESM rewrite.
 *
 * Usage: via `turbo run generate` or directly:
 *        `node packages/ui/scripts/generate-ui-registry.cjs`
 */
...
// From packages/ui/scripts/ → monorepo root is three levels up.
const ROOT = path.resolve(__dirname, '../../..')
const UI_PKG = path.join(ROOT, 'packages/ui/src/components')
const OUTPUT = path.join(
  ROOT,
  'apps/ezstart/web/src/app/[locale]/(views)/packages/ui/inspector/registry.generated.ts'
)
...
function generateOutput(registry, popularChains) {
  ...
  return `// AUTO-GENERATED by packages/ui/scripts/generate-ui-registry.cjs — ${now}
// Run: pnpm generate  (or: turbo run generate --filter=@ezstart/ui)
