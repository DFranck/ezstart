...
export interface AuditCardData {
  ...
  /** Display name */
  name: string
  /** Emoji for the audit category */
  emoji: string
  /** Short description */
  description: string
  /** Score out of 100 (null if not yet audited) */
  score: number | null
  /** Last update date string (ISO) */
  lastUpdated?: string | null
  /** Audit completion status */
  status: 'complete' | 'partial' | 'not-audited'
  /** Whether the audit data file exists (defaults to true) */
  exists?: boolean
  /** Items that have been audited */
  audited?: string[]
  /** Items that have not been audited */
  notAudited?: string[]
  /** Explanation of the score/state */
  why?: string
      ...
      <CardContent>
        <Div className="space-y-4">
          ...
          {audit.notAudited && audit.notAudited.length > 0 && (
            <Div className="space-y-2">
              <P className="text-sm font-medium text-status-degraded">Not Audited</P>
              <UL className="space-y-1">
